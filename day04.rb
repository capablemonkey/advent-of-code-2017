...
# first element is empty line
lines.shift

def part_1_valid?(passphrase)
  passphrase.each do |word|
    return false if passphrase.count(word) > 1
  end
  true
end

def part_2_valid?(passphrase)
  sorted_words = passphrase.map {|word| sort(word) }
  passphrase.each do |word|
    return false if sorted_words.count(sort(word)) > 1
  end
  true
end
...
puts(lines.count {|passphrase| part_1_valid?(passphrase) })
puts(lines.count {|passphrase| part_2_valid?(passphrase) })
# puts part_2_valid?("oiii ioii iioi iiio".split(' '))